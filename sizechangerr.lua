...
local function createAnimationListGui(animTextBox)
    ...
    mainFrame.Size = UDim2.new(0, 300, 0, 400) -- Smaller width, taller height
    mainFrame.Position = UDim2.new(0.75, -150, 0.5, -200) -- Position to the right of main GUI
    ...
    local function updateAnimationButtonsVisibility(searchText)
        ...
        for animName, animButtonData in pairs(animationButtons) do -- Changed to animButtonData
            if string.find(string.lower(animName), string.lower(searchText)) then
                animButtonData.NameButton.Visible = true -- Access NameButton
                animButtonData.PlayButton.Visible = true -- Access PlayButton
                animButtonData.NameButton.Position = UDim2.new(0, 5, 0, yOffset) -- Position NameButton
                animButtonData.PlayButton.Position = UDim2.new(0, 5, 0, yOffset + 30) -- Position PlayButton below
                yOffset = yOffset + 65 -- Increase yOffset for both buttons
                visibleButtonCount = visibleButtonCount + 1
            else
                animButtonData.NameButton.Visible = false -- Hide NameButton
                animButtonData.PlayButton.Visible = false -- Hide PlayButton
            end
        end
        animScrollFrame.CanvasSize = UDim2.new(0, 0, 0, math.max(0, yOffset - 60 + 5)) -- Adjusted CanvasSize
    end
    ...
    for animName, animId in pairs(BuiltInAnimationsR15) do
        ...
        animNameButton.Size = UDim2.new(1, -10, 0, 30)
        ...
        playAnimButton.Text = "Play Animation" -- More descriptive text
        ...
        animationButtons[animName] = {NameButton = animNameButton, PlayButton = playAnimButton} -- Store both buttons
    end
